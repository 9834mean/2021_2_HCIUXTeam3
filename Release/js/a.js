...
function Update(i) {
  Clickcategory = Number(MainData["UserHistory"][ShuffleData["Data"][i]["category"]])
  Clickcategory++
  var senddata = {
...
function GetOriginData(i) {
  ...
  //   if (xhr.readyState === XMLHttpRequest.DONE) {
  //     if (xhr.status === 200) {
  //       CheckOriginData(JSON.parse(xhr.responseText), i);
  //     } else {
  //       console.log('GetOriginData 오류로 초기페이지로 돌아갑니다.');
  //       location.href = "Index.html";
  //     }
  //   }
  // };
  ...
  $.ajax({
    type: "GET",
    url: "https://hciuxteam3-default-rtdb.firebaseio.com/NewsData/" + param + ".json",
    dataType: 'json',
    success: function (result) {
      //작업이 성공적으로 발생했을 경우
      CheckOriginData(result,i)
    },
    error: function (result) {
      //에러가 났을 경우 실행시킬 코드
      alert("CheckOriginData 오류로 초기페이지로 돌아갑니다.")
      location.href = "Index.html";
    }
  })
}

function CheckOriginData(data, i) {
  var senddata = {
    Click: 1
  };
...
function CountReadCount(i) {

  resultparam = Number(MainData["ReadCount"])
  resultparam++
